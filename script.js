const Modal = {
  open() {
    document.querySelector('.caixa-sobrepociçao').classList.add('active')
  },
  close() {
    document.querySelector('.caixa-sobrepociçao').classList.remove('active')
  }
}
...
const Transaction = {
  ...
  expense() {
    //somar saidas
    let expenses = 0
    Transaction.all.forEach(saidas => {
      if (saidas.amount < 0) {
        expenses += saidas.amount
      }
    })
    return expenses
  },
...
const DOM = {
  ...
  innerHTMLTransaction(transaction, index) {
    const CSSlcass = transaction.amount > 0 ? 'income' : 'expense'
...
const Form = {
  description: document.querySelector('input#descriçao'),
...
const App = {
  init() {
    ...
    //o DOM.addTransaction é apenas porque neles ja tem os elementos necessarios olhe na linha 86 e entenda
